---
name: solana-yieldsage-research
version: 0.1.0
description: "Use when a Solana DeFi task requires discovering, comparing, or auditing yield opportunities across Kamino, MarginFi, Jito, Orca, Raydium, Drift, and Marinade. Returns risk-adjusted APY rankings, TVL data, and cryptographically verifiable on-chain proof anchored on Solana."
---
...
Research and rank live DeFi yield opportunities on the Solana Network using the YieldSage intelligence API. This skill fetches hourly-updated APY and TVL snapshots from Kamino, MarginFi, Jito, Orca, Raydium, Drift, and Marinade, retrieves AI-generated risk-adjusted recommendations, and verifies each recommendation against its SHA-256 commitment anchored on Solana.

Use this skill when:
- A user or downstream agent needs to discover the best current yield pools on Solana
...
- Use `solana-defi-operator` when the task is execution-ready transaction building (swaps, LP adds)
- Use `solana-risk-evaluator` when the task is only a preflight risk verdict for a specific address
- Use `solana-address-registry-navigator` when the task is only address lookup or anti-phishing review
...
| `/api/recommendations/verify/{tx_hash}` | GET | Cryptographic SHA-256 verification against Solana blockchain |
| `/api/stats/overview` | GET | Aggregate TVL, average APY, protocol and pool counts |
| `/api/protocols` | GET | Full protocol registry (Kamino, MarginFi, Jito, etc.) |

## Workflow

### Mode 1: Yield Discovery (default)

Use when the user asks "what are the best yields on Solana?" or similar exploratory intent.
...
2. For each recommendation returned, present:
   ...
   - Risk tier
   - On-chain TX hash and Solscan link: `https://solscan.io/tx/{on_chain_tx_hash}`
...
Use when the user provides a Solana TX signature and asks to verify a YieldSage recommendation.
...
2. Report:
   ...
   - The Solscan link: `https://solscan.io/tx/{tx_hash}`
...
- **No address guessing**: Do not construct or guess Solana contract addresses. Use protocol names only in research outputs. Defer address resolution to `solana-address-registry-navigator`.
- **No fabricated data**: Every APY, TVL, and recommendation figure must come from a live API response in this session.
- **No execution**: This skill is research-only. Do not produce calldata, approval instructions, or transaction objects. Route execution intent to `solana-defi-operator`.
...
```
## YieldSage Yield Research — Solana Network
...
On-Chain Proof: https://solscan.io/tx/{on_chain_tx_hash}
...
- `references/on-chain-proof-guide.md` — how SHA-256 anchoring on Solana works and how to verify
