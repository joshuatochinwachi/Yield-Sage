...
async def verify_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Solana transaction signatures are Base58-encoded strings, typically 87-88 characters.
    # They do NOT start with "0x". Reject only if clearly malformed (too short/long or empty).
    if len(tx_hash) < 80 or len(tx_hash) > 100:
        await update.message.reply_text(
            "❌ Invalid Solana transaction signature format.\n"
            "Solana signatures are Base58-encoded strings, typically 87-88 characters long.\n"
            f"Your input was {len(tx_hash)} characters — please copy the full signature from Solscan.",
    ...
    try:
        ...
        if not rec_res.data:
            # Fallback to case-insensitive match (Solana Base58 is case-sensitive but allow for user copy-paste quirks)
            rec_res = (
                supabase.table("recommendations")
                .select(
                    "id, risk_tag, rank, apy_at_time, tvl_usd, ai_reasoning, ai_model, "
                ...
                .ilike("on_chain_tx_hash", f"%{tx_hash}%")
                .execute()
            )
        ...
        # Reconstruct canonical JSON payload for hash verification
        try:
        ...
        # --- TVL resolution ---
        # For new records: tvl_usd is stored in recommendations table (exact value used at hash time).
        # For old records (before this migration): tvl_usd is NULL → fall back to nearby snapshots.
        ...
        # Final fallback: build fresh payload for display even if no hash match found
        if not found_match:
            payload = build_recommendation_payload(
                protocol_name=rec["protocols"]["name"],
                pool_name=rec["protocols"]["pool_name"],
            ...
            computed_hash = hashlib.sha256(matched_payload.encode('utf-8')).hexdigest()
